...
namespace sge
{
    static std::map<HWND, Win32App*> windows;
    ...
    LRESULT CALLBACK winAppWndProc(HWND hWnd, UINT msgId, WPARAM wParam, LPARAM lParam)
    {
        Win32App* app = windows[hWnd];
        if (app) return app->wndProc(hWnd, msgId, wParam, lParam);
        return DefWindowProc(hWnd, msgId, wParam, lParam);
    }

    Win32App::Win32App(int width, int height)
        : _width(width), _height(height), _hWnd(NULL),
        _canvas(NULL), _hDC(0), _hMem(0), _buffer(0), _surface(0)
    {
        assert(registerWinClass());
    }
    ...
    int Win32App::exec()
    {
        ...
        while (WM_QUIT != msg.message)
        {
            ...
            {
                onRender(_canvas);
                ((SoftCanvas*)_canvas)->writeToBitmap(_buffer, _canvas->width(), _canvas->height());
    ...
    void Win32App::onResize(int width, int height)
    {
        ...
        else _canvas = new SoftCanvas(cw, ch);
    }

    HWND Win32App::onCreateWND()
    {
    ...
    LRESULT Win32App::wndProc(HWND hWnd, UINT msgId, WPARAM wParam, LPARAM lParam)
    {
        switch (msgId)
        {
        case WM_SIZE:
